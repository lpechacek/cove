...
//! Constructor, ip is the value of Minkowski metrics, i1..3 are the initial values.
MapColor::MapColor(int i1, int i2, int i3, double ip): x1(i1), x2(i2), x3(i3)
{
  setP(ip);
}
...
{
  double result, d;
  d = fabs(x1 - y1); result = pow(d, p);
  d = fabs(x2 - y2); result += pow(d, p);
  d = fabs(x3 - y3); result += pow(d, p);
...
MapColorRGB::MapColorRGB(double ip): MapColor(ip)
{
}

//! Constructor, calls parents' constructor, then initializes its value to i.
MapColorRGB::MapColorRGB(QRgb i, double ip): MapColor(ip)
{
  setRGBTriplet(i);
}
...
{
  x1 = qRed(i); x2 = qGreen(i); x3 = qBlue(i); 
}
...
//! Constructor, calls parents' constructor.
MapColorHSV::MapColorHSV(double ip): MapColor(ip)
{
}

//! Constructor, calls parents' constructor, then initializes its value to i.
MapColorHSV::MapColorHSV(QRgb i, double ip): MapColor(ip)
{
  setRGBTriplet(i);
}
...
{
  // the (float) is here because we need to keep hue == -1.0 true when hue is -1.0
