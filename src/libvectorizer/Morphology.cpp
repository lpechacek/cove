...
//! Constructor
Morphology::Morphology(const QImage* img): image(img), thinnedImage(0)
{
...
//! Rosenfeld thinning.
bool
Morphology::rosenfeld(ProgressObserver* progressObserver)
{
  ...
  do { // Thin image lines until there are no deletions
    ...
    for(int i = 0; i < 4; i++) {
      ...
      for (x = 0; x < xsize-1; x++)
	qb[x] = (unsigned char) (p = ((p<<1)&0006) | (!!thinnedImage->pixelIndex(x+1, 0)));
	...
	for ( x = 0 ; x < xsize-1 ; x++ ) {
	  q = qb[x];
	  p = ((p<<1)&0666) | ((q<<3)&0110) | (!!thinnedImage->pixelIndex(x+1, y+1));
    ...
    if(progressObserver)
      progressObserver->percentageChanged(100-static_cast<int>(100*
	    pow(static_cast<float>(count)/(xsize*ysize), 0.2)));
  } while(count && !(progressObserver &&
...
bool
Morphology::erosion(ProgressObserver* progressObserver)
{
...
bool
Morphology::dilation(ProgressObserver* progressObserver)
{
...
bool
Morphology::pruning(ProgressObserver* progressObserver)
{
...
bool
Morphology::runMorpholo(bool* table, bool insert, ProgressObserver* progressObserver)
{
  if(thinnedImage) delete thinnedImage;
  thinnedImage = new QImage(*image);
  thinnedImage->detach();
...
/*! Modifies thinnedImage according to given table.  Builds 3x3 neighborhood for every pixel and sets/resets (according to insert) the pixel in case the table contains true.
  \param[in] table Neighborhood table, e.g. isDeletable or isInsertable
  \param[in] insert Whether the pixel should be set or reset when the table contains true.
  \param[in] progressObserver Progress observer.
  */
int
Morphology::modifyImage(bool* table, bool insert, ProgressObserver* progressObserver)
{
  int xSize = thinnedImage->width();
  int ySize = thinnedImage->height();
  int progressHowOften = (ySize>100)?ySize/75:1;
  bool* topScanLine = new bool[xSize];
  bool* midScanLine = new bool[xSize];
  bool* botScanLine = new bool[xSize];
  int p, modifications;
  bool cancel = false;
  ...
  for(int y = 0; y < ySize-1 && !cancel; y++)
  {
    ...
    // create neighborhood
    p = topScanLine[0]<<6|topScanLine[1]<<5|
      midScanLine[0]<<3|midScanLine[1]<<2|
      botScanLine[0];
    ...
    if(table[p]) 
    {
      thinnedImage->setPixel(xSize-1, y, insert?1:0); // set the pixel
  ...
  midScanLine = botScanLine;
  botScanLine = temp;
  p = topScanLine[0]<<7|topScanLine[1]<<6|
  ...
  if(table[p]) 
  {
    thinnedImage->setPixel(xSize-1, ySize-1, insert?1:0); // set the pixel
    // TTR midScanLine[xSize-1] = false;
    modifications++;
  }
